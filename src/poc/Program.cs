...
namespace cli
{
    public static class CrdtPoc
    {
        ...
        public static async Task Main(string[] args)
        {
            ...
            {
                bool isPrimaryNode = connector?.Id.EndsWith("1") ?? throw new InvalidOperationException($"doc1(={doc1}) or connector(={connector}) is null");
                Console.WriteLine($"INIT isPrimaryNode={isPrimaryNode} doc1={doc1}: {doc1}");
                if (isPrimaryNode)
                {
                ...
                Console.WriteLine($"SET doc1={doc1}: {doc1}");
                ...
                if (!isPrimaryNode)
                {
                    ...
                    timer.Elapsed += (sender, e) =>
                    {
                        ...
                        Console.WriteLine($"TIMER doc1={doc1}: {doc1}");
                    };
                    timer.Start();
                }

                _ = Console.ReadKey();
                Console.WriteLine($"POSTEXIT doc1={doc1}: {doc1}");
            }
        }
    }
}
