                ...
                Doc.Transact(tr =>
                {
                    ...
                    for (int i = 0; i < delta.Count; i++)
                    {
                        ...
                        if (op.Insert != null)
                        {
                            ...
                            var ins = (!sanitize && op.Insert is string insertStr && i == delta.Count - 1 && curPos.Right == null && insertStr.EndsWith("\n")) ? insertStr.Substring(0, insertStr.Length - 1) : op.Insert;
                            if (!(ins is string @string) || @string.Length > 0)
                            {
        ...
        internal override void CallObserver(Transaction transaction, ISet<string> parentSubs)
        {
            ...
            if (!transaction.Local)
            {
                ...
                if (!foundFormattingItem)
                {
                    ...
                    {
                        if (item is Item it && it.Parent == this && it.Content is ContentFormat)
                        {
                            foundFormattingItem = true;
                ...
                doc.Transact(tr =>
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (item is Item it && it.Parent == this)
                            {
